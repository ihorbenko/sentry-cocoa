 ...
 */
@interface OttuSentryObjCPrivateSDKOnly : NSObject

/// For storing an envelope synchronously to disk.
+ (void)storeEnvelope:(OttuSentryObjCEnvelope *)envelope;

/// Captures an envelope and sends it to Sentry.
+ (void)captureEnvelope:(OttuSentryObjCEnvelope *)envelope;

/// Create an envelope from @c NSData. Needed for example by Flutter.
+ (nullable OttuSentryObjCEnvelope *)envelopeWithData:(NSData *)data;
...
+ (void)setTrace:(OttuSentryObjCId *)traceId spanId:(OttuSentryObjCSpanId *)spanId;
...
+ (OttuSentryObjCUser *)userWithDictionary:(NSDictionary *)dictionary;

/// Creates a @c SentryObjCBreadcrumb from a dictionary.
+ (OttuSentryObjCBreadcrumb *)breadcrumbWithDictionary:(NSDictionary *)dictionary;
